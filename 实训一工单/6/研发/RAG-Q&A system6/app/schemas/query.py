...
class QueryRequest(BaseModel):
    ...
    retrieval_mode: str | None = Field(
        default=None,
        description="本次提问使用的检索模式：fulltext / vector / hybrid / keyword",
    )
    score_threshold: float | None = Field(
        default=None,
        ge=0.0,
        description="按相关度过滤检索结果，低于阈值的片段会被丢弃",
    )
    reranker_enabled: bool | None = Field(
        default=None,
        description="是否在本次请求中启用重排",
    )
    reranker_types: list[str] | None = Field(
        default=None,
        description="本次请求使用的重排策略列表，如 tfidf、feedback、llm、cross_encoder",
    )
...
class QueryUnderstandingResult(BaseModel):
    ...
    detected_language: str = Field(default="zh", description="检测到的用户问题语言")
...
class DocumentStatusResponse(BaseModel):
    ...
    processing_message: str = "not_started"
    processing_sources: list[str] = Field(default_factory=list)
