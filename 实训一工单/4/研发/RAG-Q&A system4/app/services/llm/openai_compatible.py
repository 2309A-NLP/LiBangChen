...
6. 如果涉及收入、金额或表格数据，优先使用与问题范围最一致的表格证据。
7. 如果提供了页面图片，请结合图片中的图表、表格、标注与正文一起回答。""".strip()
...
6. If the question involves revenue, figures, or table data, prefer the table evidence that best matches the user's requested scope.
7. If page images are provided, use the charts, tables, annotations, and layout from those page images together with the retrieved passages.""".strip()
    ...
    def generate_answer(
        ...
        page_images: list[RenderedPageImage] | None = None,
    ) -> GeneratedAnswer:
        ...
        response, used_vision, vision_fallback_error = self._request_with_optional_vision(
            messages=messages,
        ...
        metadata = {
            ...
            "used_page_image_count": len(page_images or []) if used_vision else 0,
        }
        if used_vision:
            metadata["mode"] = "openai_compatible_vision"
        if vision_fallback_error:
            metadata["vision_fallback_error"] = vision_fallback_error
    ...
    def _build_user_prompt(
        ...
        *,
        ...
        if understanding.intent == "market_chart":
            if english:
                parts.append(
                    "For chart questions, first read every visible category label and its growth value from the chart area, then answer which category is highest and which is negative. Do not infer from prose if the chart labels show otherwise."
                )
            else:
                parts.append(
                    "对于图表题，请先逐项读取图中可见的类别标签及对应增长率数值，再回答哪个类别增长最快、哪个类别为负增长；如果图中标签与正文概括不完全一致，以图中标签和数值为准，不要凭常识补全。"
                )
        return "\n\n".join(parts)
    ...
    def _build_user_content(
        ...
        for page_image in page_images:
            encoded = base64.b64encode(page_image.image_bytes).decode("ascii")
            label = page_image.label or "页面图片"
            content.append(
                {
                    "type": "text",
                    "text": f"{label}：{page_image.source_id} 第 {page_image.page_number} 页",
                }
            )
            content.append(
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:{page_image.mime_type};base64,{encoded}",
                        "detail": "high",
                    },
                }
            )
    ...
    def _request_with_optional_vision(
        self,
        *,
        ...
        multimodal_messages.append(
            {
                "role": "user",
                "content": self._build_user_content(user_prompt, page_images),
            }
        )
        payload = {
            "model": self.model,
        ...
        if not page_images:
            return self._create_chat_completion(payload), False, None

        try:
            return self._create_chat_completion(payload), True, None
        except LLMRemoteError as exc:
            fallback_messages = list(messages)
            fallback_messages.append({"role": "user", "content": user_prompt})
            fallback_payload = {
                "model": self.model,
                "temperature": self.temperature,
                "messages": fallback_messages,
            }
            return self._create_chat_completion(fallback_payload), False, str(exc)
