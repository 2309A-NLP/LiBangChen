...
class RuleBasedQueryUnderstandingService:
    ...
    """
    INTENT_KEYWORDS = {
        "market_chart": ["应用结构", "增长图", "增长率", "负增长", "市场应用结构", "消费结构", "汽车电子", "ic卡"],
        "org_structure": ["组织结构图", "组织架构图", "组织架构", "架构图", "组织结构", "框图", "流程图", "销售处", "大客户销售"],
        "financial_metric": ["营收", "收入", "利润", "毛利", "毛利率", "现金流", "负债", "资产", "募集资金", "流动资金", "募投项目"],
    ...
    SECTION_MAP = {
        "market_chart": ["中国IC市场容量及消费结构", "消费结构及特点", "市场供求状况及变动原因"],
        "org_structure": ["发行人基本情况", "发行人内部组织结构图", "公司组织结构图", "公司内部职能部门、分公司简介"],
        "financial_metric": ["财务会计信息", "管理层讨论与分析", "销售情况和主要客户", "按客户群体划分的销售情况", "募集资金用途", "募集资金运用与未来发展规划"],
        "risk_factor": ["风险因素"],
    ...
    ABSTRACT_GOAL_MAP = {
        "market_chart": "定位市场应用结构图、增长率图或消费结构图，并识别最高增长、负增长及对应行业",
        "org_structure": "定位组织结构图或部门架构说明，并识别部门层级、数量及隶属关系",
    ...
    def detect_intent(self, question: str) -> str:
        ...
        if any(keyword in question for keyword in self.INTENT_KEYWORDS["market_chart"]):
            return "market_chart"
        if any(keyword in question for keyword in self.INTENT_KEYWORDS["org_structure"]):
    ...
    def _build_notes(self, question: str, intent: str) -> list[str]:
        """根据意图和问题内容生成检索备注提示。"""
        notes: list[str] = []
        if intent == "market_chart":
            notes.append("优先查找中国IC市场容量及消费结构、应用结构与增长图附近页面")
            if any(term in question for term in ("负增长", "增长率", "增长图")):
                notes.append("优先保留包含增长率、负增长、汽车电子、IC卡的图表页面")
        if intent == "org_structure":
            notes.append("优先查找组织结构图、内部职能部门说明和分公司简介附近页面")
            if any(term in question for term in ("销售部", "大客户销售", "销售处")):
                notes.append("优先保留包含销售部、大客户销售、销售处的组织架构相关页面")
        if intent == "financial_metric":
            if any(term in question for term in ("军用", "国防", "民用", "车用", "汽车", "客户群体")):
                notes.append("优先查找按客户群体划分的销售情况表格")
            if any(term in question for term in ("募集资金", "募投项目", "补充流动资金", "补充营运资金")):
                notes.append("优先查找募集资金用途、募集资金运用与未来发展规划中的募投项目表格")
    ...
    def _is_financial_metric_question(self, question: str) -> bool:
        ...
        financial_terms = {"收入", "营收", "主营业务收入", "销售金额", "金额", "占比", "比例", "毛利", "利润", "募集资金", "流动资金", "募投项目"}
        table_terms = {"军用", "国防", "军队", "民用", "车用", "汽车", "客户群体", "前五名客户", "前五大客户", "报告期", "2016", "2017", "2018", "分别", "募集资金用途", "募集资金运用", "补充流动资金", "补充营运资金", "本次发行"}
