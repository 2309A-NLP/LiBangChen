...
class DocumentIngestionService:
    ...
    def load_document(self, force: bool = False) -> None:
        ...
        try:
            for pdf_path in pdf_paths:
                ...
                self._source_paths[pdf_path.name] = pdf_path
                try:
    ...
    def render_page_image(self, source_id: str, page_number: int) -> RenderedPageImage | None:
        """将指定 PDF 页渲染为图片，供多模态模型二阶段识别。"""
        if page_number < 1:
            return None

        pdf_path = self._resolve_source_path(source_id)
        ...
        try:
            with fitz.open(pdf_path) as document:
                ...
                matrix = fitz.Matrix(
                    self.settings.llm_vision_render_scale,
                    self.settings.llm_vision_render_scale,
                )
    ...
    def _resolve_source_path(self, source_id: str) -> Path | None:
        """根据 source_id 解析实际 PDF 路径。"""
        source_path = self._source_paths.get(source_id)
        if source_path is not None:
            return source_path

        candidate = self.settings.source_pdf_dir / source_id
        if candidate.exists():
            return candidate

        fallback = self.settings.source_pdf_path
        if fallback.exists() and fallback.name == source_id:
            return fallback
        return None
