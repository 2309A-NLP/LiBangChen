...
def test_openai_compatible_llm_client_sends_page_images(monkeypatch):
    client = OpenAICompatibleLLMClient(
        api_key="test-key",
        base_url="https://example.com/v1",
        model="test-model",
    ...
    result = client.generate_answer(
        ...
        understanding=QueryUnderstandingResult(
            ...
            normalized_question="图表里收入趋势是什么？",
            strategy="online",
            sub_questions=[],
            abstracted_goal="读取图表并概括趋势。",
    ...
    assert result.metadata["mode"] == "openai_compatible_vision"
    assert result.metadata["used_page_image_count"] == 1
