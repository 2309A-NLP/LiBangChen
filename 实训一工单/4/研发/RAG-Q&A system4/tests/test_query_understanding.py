...
def test_query_understanding_detects_financial_metric_for_vehicle_domain_revenue_question():
    service = QueryUnderstandingService(
        Settings(query_understanding_mode="online", query_understanding_local_first_enabled=True)
    )

    result = service.understand("报告期内，武汉兴图新科电子股份有限公司来自车用领域的收入分别是多少")

    assert result.intent == "financial_metric"
    assert "车用领域" in result.retrieval_hints["keywords"]
    assert "民用领域" in result.retrieval_hints["keywords"]
    assert "按客户群体划分的销售情况" in result.retrieval_hints["prefer_sections"]


def test_query_understanding_detects_financial_metric_for_fundraising_use_question():
    service = QueryUnderstandingService(
        Settings(query_understanding_mode="online", query_understanding_local_first_enabled=True)
    )
    ...
    assert "募集资金运用与未来发展规划" in result.retrieval_hints["prefer_sections"]
