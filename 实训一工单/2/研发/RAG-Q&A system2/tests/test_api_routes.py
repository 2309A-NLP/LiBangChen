...
def test_document_select_and_stream_query(tmp_path, monkeypatch):
    ...
    class StubDocumentTaskService:
        def __init__(self) -> None:
            self.tasks = {}
            self.counter = 0

        def create_task(self, uploaded_files):
            self.counter += 1
            task = {
        ...
        def start(self, task_id, *, total_files, message):
        ...
        def fail(self, task_id, *, current_step, message, error):
            task = self.tasks[task_id]
            task["status"] = "failed"
            task["current_step"] = current_step
            task["message"] = message
    ...
    stub_container = SimpleNamespace(
        ...
        document_task_service=StubDocumentTaskService(),
    ...
    stub_container.document_ingestion_service.ocr_service = SimpleNamespace(
    ...
    with patch("app.api.routes.Thread") as thread_cls, TestClient(app) as client:
        ...
        assert upload_payload["uploaded_files"] == ["new.pdf"]
        assert prepare_calls["count"] == 1

        task_response = client.get(f"/api/document/tasks/{upload_payload['task_id']}")
        assert task_response.status_code == 200
        assert task_response.json()["status"] == "success"
        assert task_response.json()["selected_sources"] == ["new.pdf"]

        missing_task_response = client.get("/api/document/tasks/task-missing")
        ...
        ocr_status_response = client.get("/api/document/ocr/status")
        assert ocr_status_response.status_code == 200
        assert ocr_status_response.json()["enabled"] is True
        assert ocr_status_response.json()["available"] is False
