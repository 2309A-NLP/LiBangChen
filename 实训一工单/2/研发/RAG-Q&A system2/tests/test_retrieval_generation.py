...
def test_retrieval_generation_extracts_fundraising_usage_amount_locally():
    service = RetrievalGenerationService(
        retriever=StubRetriever(
            [
                RetrievedChunk(
                    chunk=DocumentChunk(
                        ...
                        page_number=261,
                        text=(
                            "本次发行募集资金投资项目及拟投入募集资金金额如下："
                            "补充流动资金项目拟使用募集资金金额为 12,000.00 万元。"
                        ),
                    ),
                    score=0.91,
                )
    ...
    response = service.answer(
        question="计划使用本次发行募集资金的多少用于补充流动资金？",
        understanding=QueryUnderstandingResult(
            intent="financial_metric",
            normalized_question="计划使用本次发行募集资金的多少用于补充流动资金？",
            strategy="rules",
            sub_questions=[],
            abstracted_goal="定位募集资金用途中补充流动资金对应的金额。",
            retrieval_hints={"keywords": ["募集资金", "补充流动资金", "募投项目"]},
        ),
    )

    assert response.answer == "本次发行募集资金中，用于补充流动资金的金额为 12,000.00 万元。"
    assert "补充流动资金项目拟使用募集资金金额为 12,000.00 万元" in response.citations[0].snippet
