...
class RuleBasedQueryUnderstandingService:
    ...
    def build_retrieval_hints(
        ...
        if any(term in question for term in ("组织结构", "组织架构", "结构图", "架构图", "销售处")):
            prefer_sections = self._merge_unique(
                prefer_sections,
                ["发行人组织结构", "公司内部职能部门、分公司简介", "机构独立"],
            )
    ...
    def _extract_keywords(self, question: str, entities: list[str] | None = None) -> list[str]:
        ...
        explicit_keywords = self._build_explicit_financial_keywords(working_text)
        explicit_keywords = self._merge_unique(
            explicit_keywords,
            self._build_explicit_org_chart_keywords(working_text),
        )
        return self._merge_unique(preferred_terms, explicit_keywords)
    ...
    def _build_notes(self, question: str, intent: str) -> list[str]:
        ...
        if any(term in question for term in ("组织结构", "组织架构", "结构图", "架构图", "销售处")):
            notes.append("优先查找组织结构图及公司内部职能部门说明")
        return notes
    ...
    def _is_financial_metric_question(self, question: str) -> bool:
        ...
        if "前五" in normalized_question and "客户" in normalized_question:
            return any(term in normalized_question for term in financial_terms)
        return any(term in question for term in financial_terms) and any(
            term in normalized_question for term in table_terms
        )

    def _build_explicit_financial_keywords(self, question: str) -> list[str]:
        """为财务表格类问题补充稳定的中文检索词，避免别名漏召回。"""
        keywords: list[str] = []

        revenue_terms = ("收入", "营收", "主营业务收入", "销售金额", "金额", "占比", "比例")
    ...
    def _build_explicit_org_chart_keywords(self, question: str) -> list[str]:
        """为组织结构图相关问题补充稳定检索词。"""
        chart_terms = ("组织结构", "组织架构", "结构图", "架构图")
        if not any(term in question for term in (*chart_terms, "销售处", "销售部")):
            return []
...
class QueryUnderstandingService:
    ...
    def contextualize_question(
        ...
        rewritten_question = self._rewrite_followup_locally(normalized_question, history)
