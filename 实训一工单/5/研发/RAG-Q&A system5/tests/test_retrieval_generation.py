...
def test_retrieval_generation_maps_military_aliases_to_defense_row():
    service = RetrievalGenerationService(
        retriever=StubRetriever(
            [
                RetrievedChunk(
    ...
    response = service.answer(
        question="报告期内，公司来自军工领域的收入分别是多少？",
        understanding=QueryUnderstandingResult(
            intent="financial_metric",
            normalized_question="报告期内，公司来自军工领域的收入分别是多少？",
            strategy="rules",
            sub_questions=[],
            abstracted_goal="定位报告期内军工领域收入并按年度给出金额。",
            retrieval_hints={"keywords": ["收入", "军工领域", "国防领域", "客户群体"]},
        ),
    )
...
def test_retrieval_generation_runtime_fallback_promotes_misclassified_financial_question():
    service = RetrievalGenerationService(
        retriever=StubRetriever(
            [
                RetrievedChunk(
    ...
    response = service.answer(
        ...
        understanding=QueryUnderstandingResult(
            ...
            strategy="rules",
            sub_questions=[],
            abstracted_goal="从招股说明书中定位相关事实并作答。",
            retrieval_hints={"keywords": [], "prefer_sections": ["发行人基本情况", "招股说明书摘要"]},
        ),
    )

    assert response.understanding.intent == "financial_metric"
    assert response.understanding.retrieval_hints["runtime_fallback"] == "financial_metric"
    assert "国防领域" in response.understanding.retrieval_hints["keywords"]
    assert "2016年 6,464.51 万元" in response.answer


def test_retrieval_generation_extracts_sales_offices_from_org_chart():
    service = RetrievalGenerationService(
        retriever=StubRetriever(
            [
                RetrievedChunk(
                    chunk=DocumentChunk(
                        ...
                        page_number=39,
                        text=(
                            "发行人内部组织结构图 销售部 电话及网络销售部 渠道销售部 大客户销售部 国际贸易部 "
                            "珠海销售处 深圳销售处 北京销售处 武汉销售处 广州销售处 成都销售处"
                        ),
                    ),
                    score=0.95,
                )
    ...
    response = service.answer(
        question="组织结构图里，哪个销售部的销售处最多？有哪些销售处？",
        understanding=QueryUnderstandingResult(
            intent="general_information",
            normalized_question="组织结构图里，哪个销售部的销售处最多？有哪些销售处？",
            strategy="rules",
            sub_questions=[],
            abstracted_goal="定位组织结构图中的销售部门及销售处信息。",
            retrieval_hints={"keywords": ["组织结构图", "销售部", "销售处"]},
        ),
    )

    assert "渠道销售部对应的销售处最多" in response.answer
    assert "珠海销售处" in response.answer
    assert "成都销售处" in response.answer
