...
class RetrievalGenerationService:
    ...
    def _try_local_extraction(
        ...
        if self._looks_like_fundraising_use_question(question, understanding):
            extracted = self._extract_fundraising_use(retrieved_chunks)
            if extracted:
                return LocalExtractionResult(
                    answer=extracted,
                    metadata={"mode": "local_extraction", "pattern": "fundraising_use"},
                )
    ...
    def _looks_like_fundraising_use_question(
        self,
        question: str,
        understanding: QueryUnderstandingResult,
    ) -> bool:
        normalized_question = question.replace(" ", "")
        if understanding.intent == "fundraising_use":
            return True
        return any(
            term in normalized_question
            for term in ("募集资金", "募投", "募投项目", "资金用途", "补充流动资金")
        )

    def _extract_fundraising_use(
        self,
        retrieved_chunks: list[RetrievedChunk],
    ) -> str | None:
        patterns = [
            re.compile(
                r"(?:拟|计划)?(?:使用|投入)?(?:本次)?(?:发行)?募集资金(?:总额)?(?:中)?[^。；\n]{0,48}?([0-9][0-9,]*(?:\.\d+)?)\s*(亿元|万元|元)[^。；\n]{0,48}?补充流动资金"
            ),
            re.compile(
                r"补充流动资金[^。；\n]{0,48}?([0-9][0-9,]*(?:\.\d+)?)\s*(亿元|万元|元)"
            ),
        ...
        for item in retrieved_chunks:
            normalized_text = re.sub(r"\s+", " ", item.chunk.text).strip()
            for pattern in patterns:
                match = pattern.search(normalized_text)
                if not match:
                    continue
                amount, unit = match.groups()
                return f"公司计划将本次发行募集资金中的 {amount} {unit} 用于补充流动资金。"
