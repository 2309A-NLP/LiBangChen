...
class DocumentIngestionService:
    ...
    NO_TEXT_WARNING = "No extractable text was found in any discovered PDF."
    ...
    def load_document(self, force: bool = False) -> None:
        ...
        self._chunks = []
        self._warnings = []
        self._source_files = [path.name for path in pdf_paths]
        ...
        try:
            self._chunks, self._warnings = self._extract_chunks_from_paths(pdf_paths, PdfReader)
        finally:
            pypdf_logger.setLevel(previous_level)

        if not self._chunks:
            self._warnings.append(self.NO_TEXT_WARNING)

        self._selected_sources &= set(self._source_files)
        self._last_loaded_at = datetime.now(timezone.utc).isoformat()

    def load_uploaded_documents(self, source_files: list[str]) -> None:
        """Incrementally reload only uploaded PDFs instead of reparsing the entire source dir."""
        if not source_files:
            return

        if not self._source_files and not self._chunks and self._last_loaded_at is None:
            self.load_document(force=True)
            return

        discovered_paths = self._discover_pdf_paths()
        discovered_map = {path.name: path for path in discovered_paths}
        requested_names = list(dict.fromkeys(source_files))
        requested_set = set(requested_names)
        target_paths = [discovered_map[name] for name in requested_names if name in discovered_map]

        if not target_paths:
            self._source_files = [path.name for path in discovered_paths]
            self._selected_sources &= set(self._source_files)
            self._last_loaded_at = datetime.now(timezone.utc).isoformat()
            return
        ...
        try:
            fresh_chunks, fresh_warnings = self._extract_chunks_from_paths(target_paths, PdfReader)
        finally:
            pypdf_logger.setLevel(previous_level)

        self._chunks = [chunk for chunk in self._chunks if chunk.source_id not in requested_set]
        ...
        retained_warnings = [
            ...
            if warning != self.NO_TEXT_WARNING
            and not any(name in warning for name in requested_names)
        ]
        self._warnings = retained_warnings + fresh_warnings
        if not self._chunks:
            self._warnings.append(self.NO_TEXT_WARNING)
    ...
    def _extract_chunks_from_paths(
        self,
        pdf_paths: list[Path],
        pdf_reader_cls,
        ...
        for pdf_path in pdf_paths:
            try:
                ...
                warnings.append(f"Failed to read PDF {pdf_path.name}: {exc}")
                continue
