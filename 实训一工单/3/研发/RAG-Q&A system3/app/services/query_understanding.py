...
class RuleBasedQueryUnderstandingService:
    ...
    INTENT_KEYWORDS = {
        ...
        "fundraising_use": ["募集资金", "募投", "募投项目", "资金用途", "补充流动资金", "流动资金"],
    ...
    SECTION_MAP = {
        ...
        "fundraising_use": ["募集资金运用", "本次募集资金运用", "募集资金投资项目"],
        "risk_factor": ["风险因素"],
    ...
    ABSTRACT_GOAL_MAP = {
        ...
        "fundraising_use": "定位募集资金用途、补充流动资金金额或募投项目安排，并直接回答对应金额或用途",
    ...
    def _extract_entities(self, question: str) -> list[str]:
        ...
        entities = [
            normalized
            for match in self.ENTITY_PATTERN.finditer(sanitized_question)
            if (normalized := self._normalize_entity_name(match.group(0)))
        ]
    ...
    def _build_notes(self, question: str, intent: str) -> list[str]:
        ...
        if intent == "fundraising_use":
            notes.append("优先查找募集资金运用、募集资金投资项目、补充流动资金相关段落或表格")
        return notes
