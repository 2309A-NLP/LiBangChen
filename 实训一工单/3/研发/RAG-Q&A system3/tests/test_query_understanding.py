...
def test_query_understanding_normalizes_company_entity_prefix_noise():
    service = QueryUnderstandingService(Settings(query_understanding_mode="rules"))

    result = service.understand("与武汉力源信息技术股份有限公司在控制关系的关联方是谁？")

    assert result.retrieval_hints["entities"] == ["武汉力源信息技术股份有限公司"]
...
def test_query_understanding_detects_fundraising_use_question():
    service = QueryUnderstandingService(Settings(query_understanding_mode="rules"))

    result = service.understand("武汉兴图新科电子股份有限公司计划使用本次发行募集资金的多少用于补充流动资金？")

    assert result.intent == "fundraising_use"
    assert "募集资金" in result.retrieval_hints["keywords"]
    assert "补充流动资金" in result.retrieval_hints["keywords"]
    assert result.retrieval_hints["prefer_sections"] == ["募集资金运用", "本次募集资金运用", "募集资金投资项目"]
...
def test_pipeline_allows_retrieval_when_entity_matches_after_normalization():
    understanding = QueryUnderstandingResult(
        intent="shareholding",
        normalized_question="与武汉力源信息技术股份有限公司在控制关系的关联方是谁？",
        strategy="rules",
        ambiguous_terms=[],
        clarification_needed=False,
        clarification_question=None,
        sub_questions=[],
        abstracted_goal="定位控制关系关联方并直接回答。",
        assumptions=[],
        retrieval_hints={
            "keywords": ["控制关系", "关联方"],
            "entities": ["与武汉力源信息技术股份有限公司"],
        },
    )

    class StubQueryUnderstandingService:
        def understand(self, question: str) -> QueryUnderstandingResult:
            assert question == "与武汉力源信息技术股份有限公司在控制关系的关联方是谁？"
            return understanding

    class StubRetrievalGenerationService:
        def answer(self, **kwargs: object):
            assert kwargs["question"] == "与武汉力源信息技术股份有限公司在控制关系的关联方是谁？"

            class Response:
                pass

            response = Response()
            response.answer_id = "resp-1"
            response.session_id = ""
            response.answer = "mock answer"
    ...
    class StubDocumentIngestionService:
        ...
        def status(self):
            return {
                "selected_sources": ["招股说明书2.pdf"],
                "source_files": ["招股说明书2.pdf"],
            }
    ...
    response = pipeline.answer_question(
        QueryRequest(question="与武汉力源信息技术股份有限公司在控制关系的关联方是谁？", include_debug=True)
    )
...
def test_pipeline_reports_selected_document_scope_mismatch_before_retrieval():
    understanding = QueryUnderstandingResult(
        intent="general_information",
        normalized_question="武汉力源信息技术股份有限公司本次发行股数是多少？",
        strategy="rules",
        ambiguous_terms=[],
        clarification_needed=False,
        clarification_question=None,
        sub_questions=[],
        abstracted_goal="定位本次发行股数并直接回答。",
        assumptions=[],
        retrieval_hints={
            "keywords": ["本次发行", "发行股数"],
    ...
    class StubQueryUnderstandingService:
        def understand(self, question: str) -> QueryUnderstandingResult:
            assert question == "武汉力源信息技术股份有限公司本次发行股数是多少？"
            return understanding

    class StubRetrievalGenerationService:
        def answer(self, **_: object):
            raise AssertionError("document scope mismatch should not call retrieval_generation_service")
    ...
    class StubDocumentIngestionService:
        def chunks(self):
            return [
                type(
                    ...
                    (),
                    {
                        "text": "武汉兴图新科电子股份有限公司 本次发行概况 发行股数不低于1840万股",
                    },
                )()
            ]
        ...
        def status(self):
            return {
                "selected_sources": ["招股说明书1-无水印.pdf"],
    ...
    response = pipeline.answer_question(
        QueryRequest(question="武汉力源信息技术股份有限公司本次发行股数是多少？", include_debug=True)
    )

    assert "未检索到“武汉力源信息技术股份有限公司”" in response.answer
    assert "武汉兴图新科电子股份有限公司" in response.answer
    assert response.citations == []
    assert response.debug["mode"] == "document_scope_mismatch"
