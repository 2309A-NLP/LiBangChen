...
def test_load_uploaded_documents_only_reparses_uploaded_files(tmp_path, monkeypatch):
    source_dir = tmp_path / "source"
    source_dir.mkdir()
    (source_dir / "a.pdf").write_bytes(b"%PDF-1.4")
    (source_dir / "b.pdf").write_bytes(b"%PDF-1.4")
    ...
    class FakeReader:
        def __init__(self, path: str) -> None:
            ...
            self.pages = [FakePage(text) for text in page_text_by_name[name]]

    monkeypatch.setattr("pypdf.PdfReader", FakeReader)

    service.load_document(force=True)
    (source_dir / "c.pdf").write_bytes(b"%PDF-1.4")

    service.load_uploaded_documents(["c.pdf"])
    status = service.status()

    assert read_counts == {"a.pdf": 1, "b.pdf": 1, "c.pdf": 1}
    assert status["source_files"] == ["a.pdf", "b.pdf", "c.pdf"]
    assert status["document_count"] == 3
    assert status["chunk_count"] == 3
    assert {chunk.source_id for chunk in service.all_chunks()} == {"a.pdf", "b.pdf", "c.pdf"}
