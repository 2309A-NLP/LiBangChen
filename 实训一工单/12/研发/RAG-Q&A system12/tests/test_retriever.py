...
def test_retriever_factory_returns_lightrag_retriever_without_vector_setup():
    from app.services.retrievers.lightrag import LightRAGRetriever

    ingestion = StubDocumentIngestionService([])
    retriever = build_retriever(Settings(retriever_type="hybrid"), ingestion, "lightrag_mix")

    assert isinstance(retriever, LightRAGRetriever)


def test_lightrag_retriever_maps_selected_sources_and_filters_results():
    from app.services.retrievers.lightrag.retriever import LightRAGRetriever
        ...
        def query(self, request):
            ...
            return LightRAGQueryResponse(
                ...
                chunks=[
                    LightRAGChunk(
                        chunk_id="keep",
                        content="important graph context",
                        score=0.8,
                        file_id="prospectus_2024",
    ...
    retriever = LightRAGRetriever(
        settings=Settings(lightrag_min_score=0.5).lightrag,
        document_ingestion_service=ingestion,
        retrieval_mode="lightrag_global",
        client=client,
    )

    results = retriever.retrieve("who controls the company?", top_k=5)

    assert client.requests[0].mode == LightRAGQueryMode.GLOBAL
    assert client.requests[0].file_ids == ["prospectus_2024"]
    assert [item.chunk.chunk_id for item in results] == ["keep"]
    assert results[0].chunk.page_number == 7
    assert results[0].metadata["lightrag_answer"] == "graph answer"
    assert results[0].metadata["lightrag_total_tokens"] == 123
...
def test_lightrag_client_uses_actual_insert_and_query_data_endpoints():
    ...
    class FakeHTTPClient:
        ...
        def post(self, path, json=None, timeout=None):
            ...
            if path == "/documents/text":
                return FakeResponse({"status": "success", "track_id": "t1"})
            return FakeResponse(
                {
                    "status": "success",
                    "data": {
                        "entities": [
                            {
                                ...
                                "description": "实际控制人",
                                "source_id": "e1",
                                "file_source": "prospectus_2024.pdf",
                                "weight": 0.9,
                            }
                        ],
                        "relationships": [
    ...
    insert_result = client.insert(LightRAGInsertRequest(text="hello", file_id="prospectus_2024"))
    query_result = client.query(
        LightRAGQueryRequest(
            query="实际控制人是谁？",
            mode=LightRAGQueryMode.MIX,
            file_ids=["prospectus_2024"],
        )
    )

    assert insert_result["status"] == "success"
    assert fake_http.posts[0][0] == "/documents/text"
    assert fake_http.posts[0][1] == {"text": "hello", "file_source": "prospectus_2024"}
    assert fake_http.posts[1][0] == "/query/data"
    assert fake_http.posts[1][1] == {"query": "实际控制人是谁？", "mode": "mix"}
    assert [chunk.metadata["lightrag_type"] for chunk in query_result.chunks] == [
        "entity",
        "relationship",
    ]
    assert query_result.chunks[0].file_id == "prospectus_2024.pdf"
    assert query_result.chunks[1].file_id == "prospectus_2024"


def test_lightrag_retriever_filters_sources_and_marks_unavailable_source():
    from app.services.retrievers.lightrag.retriever import LightRAGRetriever
    from app.services.retrievers.lightrag.schemas import (
        LightRAGChunk,
        LightRAGQueryMode,
    ...
    retriever = LightRAGRetriever(
        settings=Settings(lightrag_min_score=0.0).lightrag,
    ...
    results = retriever.retrieve(
        "test",
        top_k=10,
        retrieval_hints={"source_files": ["prospectus_2024.pdf"]},
    )

    assert [item.chunk.chunk_id for item in results] == ["match", "unknown"]
    assert results[0].metadata["lightrag_source_filter_matched"] is True
    assert results[1].metadata["lightrag_source_filter_unavailable"] is True
