...
4. 不要编造页码、数字、时间或公司事实。
5. 输出纯文本，不使用 markdown 格式（不加粗、不斜体、不用列表符号、不用标题）。
6. 如果涉及收入、金额或表格数据，优先使用与问题范围最一致的表格证据。
7. 列举信息时使用中文序号（一、二、三）或顿号分隔。""".strip()
...
5. Output plain text only — no markdown formatting (no bold, no italic, no list markers, no headings).
...
7. When listing items, use plain text — separate with commas or semicolons, do not start lines with markers.""".strip()
