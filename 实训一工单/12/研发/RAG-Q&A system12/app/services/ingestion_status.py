...
class IngestionStatusService:
    ...
    def start_lightrag_sync(self, message: str) -> None:
        with self._lock:
            self._state["lightrag_status"] = "syncing"
            self._state["lightrag_error"] = None

    def lightrag_synced(self) -> None:
        with self._lock:
            self._state["lightrag_status"] = "synced"
            self._state["lightrag_error"] = None

    def lightrag_failed(self, error: str) -> None:
        with self._lock:
            self._state["lightrag_status"] = "failed"
            self._state["lightrag_error"] = error
