...
class RetrievalGenerationService:
    ...
    def _extract_annual_report_fact(
        ...
        if "组织结构图" in normalized_question and "销售处" in normalized_question:
            return self._extract_sales_office_structure_fact(source_chunks, question)
    ...
    def _extract_sales_office_structure_fact(
        ...
        if not chunks:
            return None
        # 提取问题中的公司名，校验匹配的 chunk 是否属于该公司
        company_keywords = self._extract_company_keywords(question)
        if company_keywords:
            chunks = self._filter_chunks_by_company(chunks, company_keywords)
            if not chunks:
                return None
    ...
    def _try_chart_vision_fallback(
        ...
        # 一次性解析 source_files，后续策略复用
        source_files: list[str] = self.document_ingestion_service.available_source_files()
        if not source_files:
        ...
        # 策略 C：文本评分全空时（图表是图片，pypdf 抽不到文字），
        # 直接用问题关键词在全部 PDF 页面的文本中做模糊匹配，找最相关的页面
        if not candidate_pages:
            for pdf_name in source_files:
                page_texts = self.document_ingestion_service.get_page_texts(pdf_name)
                if not page_texts:
                    continue
                # 用问题中的单个词（≥2字）在所有页面中匹配
                loose_terms: list[str] = []
                for term in question_terms:
                    if len(term) >= 2:
                        loose_terms.append(term)
                if not loose_terms:
                    # 从问题本身提取 2-4 字片段
                    q = self._normalize_match_text(question)
                    loose_terms = [q[i:i+2] for i in range(len(q)-1)] + [q[i:i+3] for i in range(len(q)-2)] if len(q) >= 4 else []
                page_hits: dict[int, int] = {}
                for page_number, text in page_texts:
                    norm = self._normalize_match_text(text)
                    hits = sum(1 for t in loose_terms if t in norm)
                    if hits > 0:
                        page_hits[page_number] = hits
                if page_hits:
                    sorted_pages = sorted(page_hits, key=lambda p: -page_hits[p])
                    candidate_pages[pdf_name] = sorted_pages[: self.CHART_FALLBACK_MAX_PAGES_PER_PDF]

        # 策略 D：用问题中的具体关键词（年份、专有名词）精确匹配含图表的页面
        for pdf_name in source_files:
            page_texts = self.document_ingestion_service.get_page_texts(pdf_name)
            if not page_texts:
                continue
            text_lens = [len(text) for _, text in page_texts]
            if not text_lens:
                continue
            text_lens.sort()
            median_len = text_lens[len(text_lens) // 2]
            existing = set(candidate_pages.get(pdf_name, []))
            # 从 focus_terms 和 question_terms 中提取子串用于匹配
            ...
            for ft in focus_terms:
                if len(ft) >= 2:
                    all_search_terms.add(ft)
                    # 额外拆成 2-3 字短子串，提高匹配率
                    for i in range(len(ft) - 1):
                        all_search_terms.add(ft[i:i+2])
            chart_page_scores: dict[int, int] = {}
            for page_number, text in page_texts:
                ...
                score = 0
                # 包含图表标记
                if any(m in norm for m in ("图", "表", "图表", "亿元")):
                    score += 2
                # 子串匹配
                hit_count = sum(1 for t in all_search_terms if len(t) >= 2 and t in norm)
                score += min(hit_count, 10)
                # 文字量偏低 → 可能是图表页
                if len(text) < max(median_len * 0.4, 50):
                    score += 1
                if score >= 3:
                    chart_page_scores[page_number] = score
            sorted_pages = sorted(chart_page_scores, key=lambda p: -chart_page_scores[p])
            extra = sorted_pages[: self.CHART_FALLBACK_MAX_PAGES_PER_PDF]
            if extra:
                candidate_pages.setdefault(pdf_name, []).extend(extra)
                logger.info("Chart fallback D: %s pages %s", pdf_name, extra)
        if not candidate_pages:
            return None
    ...
    def _extract_company_keywords(question: str) -> list[str]:
        ...
        for suffix in suffixes:
            ...
            if idx > 0:
                full_name = question[:idx + len(suffix)]
                # 取简称：去掉后缀，再取最后 2-4 个字
                short = full_name[:idx]
                if len(short) >= 2:
                    keywords.append(short[-4:])  # 如 "武汉力源" "武汉兴图"
                    keywords.append(short[-2:])  # 如 "力源" "兴图"
                break
        return [kw for kw in keywords if kw]

    @staticmethod
    def _filter_chunks_by_company(
        chunks: list[DocumentChunk],
        company_keywords: list[str],
    ) -> list[DocumentChunk]:
        """过滤出包含公司关键词的 chunk，至少命中一个关键词."""
        return [
            chunk for chunk in chunks
            if any(kw in chunk.text for kw in company_keywords)
        ]
