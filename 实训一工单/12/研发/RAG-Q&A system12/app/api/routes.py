...
def _format_stream_error(exc: Exception) -> str:
    ...
    if "lightrag server unavailable" in normalized or "lightrag query failed" in normalized:
        return "LightRAG Server 暂不可用，请确认 sidecar 已启动，或将 retrieval_mode 切换为 hybrid / fulltext / vector / keyword。"
    return message
    ...
    *,
    ...
    def runner() -> None:
        try:
            ...
            # Sync newly uploaded documents to LightRAG knowledge graph (best-effort)
            container.ingestion_status_service.start_lightrag_sync(
                "Syncing documents to LightRAG knowledge graph..."
            )
            try:
                container.sync_lightrag(source_files=source_files)
            except Exception:
                container.ingestion_status_service.lightrag_failed(
                    "LightRAG sidecar unavailable or sync failed; "
                    "documents are still available in standard retrieval modes."
                )
                logger.warning("LightRAG sync failed in background.", exc_info=True)
            else:
                container.ingestion_status_service.lightrag_synced()
