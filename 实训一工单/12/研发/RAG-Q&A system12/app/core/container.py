...
from app.services.retrievers.lightrag.indexer import LightRAGIndexer
...
class AppContainer:
    ...
    def __init__(
        ...
        lightrag_indexer: LightRAGIndexer | None = None,
    ) -> None:
        ...
        self.lightrag_indexer = lightrag_indexer
    ...
    def sync_lightrag(self, source_files: list[str] | None = None) -> None:
        """Sync newly uploaded PDFs to the LightRAG sidecar knowledge graph.

        Runs health check first — if the sidecar is unreachable, skips silently.
        Failures are logged but never raised, so the main upload flow is unaffected.
        """
        if self.lightrag_client is None or self.lightrag_indexer is None:
            logger.info("LightRAG is not configured; skipping graph sync.")
            return

        if not self.lightrag_client.health_check():
            logger.info("LightRAG sidecar is not reachable; skipping graph sync.")
            return

        pdf_paths = self.document_ingestion_service.get_pdf_paths(source_files or [])
        if not pdf_paths:
            logger.info("No PDF paths found for LightRAG sync.")
            return

        try:
            self.lightrag_indexer.index_documents(
                pdf_paths,
                incremental=True,
                force_rebuild=False,
            )
            logger.info("LightRAG graph sync completed for %s document(s).", len(pdf_paths))
        except Exception:
            logger.warning("LightRAG graph sync failed.", exc_info=True)
            raise
    ...
    def build(cls) -> "AppContainer":
        ...
        # LightRAG sidecar components (may be unavailable — will degrade gracefully)
        lightrag_settings = settings.lightrag
        try:
            lightrag_client = LightRAGClient(
                base_url=lightrag_settings.base_url,
                api_key=lightrag_settings.api_key,
                timeout=lightrag_settings.timeout,
                max_retries=lightrag_settings.insert_retry,
            )
            lightrag_indexer = LightRAGIndexer(
                settings=lightrag_settings,
                client=lightrag_client,
            )
        except Exception:
            logger.warning("Failed to initialize LightRAG components.", exc_info=True)
