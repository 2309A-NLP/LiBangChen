...
def test_query_understanding_contextualizes_followup_locally():
    ...
    assert rewritten_question == "武汉兴图新科电子股份有限公司的主营业务是什么？"
...
def test_query_understanding_contextualizes_entity_switch_followup_locally():
    service = QueryUnderstandingService(Settings(query_understanding_mode="rules"))

    rewritten_question, debug = service.contextualize_question(
        "那武汉力源信息技术股份有限公司呢？",
        [
            {"role": "user", "content": "武汉兴图新科电子股份有限公司的法定代表人是谁？"},
            {"role": "assistant", "content": "武汉兴图新科电子股份有限公司的法定代表人是程家明。"},
        ],
    )

    assert rewritten_question == "武汉力源信息技术股份有限公司的法定代表人是谁？"
    assert debug["rewritten"] is True
    assert debug["source"] == "local"
